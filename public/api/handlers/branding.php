<?php
/**
 * Handler branding: logotipo de la app (solo administrador) y tema de color por
 * usuario (cualquiera). Configuración es un módulo universal — sin permiso de
 * módulo que filtre por rol — así que el chequeo de administrador para el logo
 * vive aquí adentro, no en el ruteo (mismo patrón que handle_calendar()).
 */
...
function handle_branding(string $action): void
{
    ...
    switch ($action) {
        ...
        case 'upload_logo': {
            ...
            $name = 'logo-' . date('YmdHis') . '-' . bin2hex(random_bytes(3)) . '.' . $ext;
            ...
            // El favicon y el manifest de PWA necesitan tamaños cuadrados fijos —
            // se generan aquí una sola vez a partir del logo recién subido, así
            // el admin no tiene que preparar tres archivos por separado.
            $icon192 = branding_make_icon($dest, 192, 'icon-192');
            $icon512 = branding_make_icon($dest, 512, 'icon-512');

            branding_unlink_all(branding_config());
            branding_save(['logo_file' => $name, 'icon_192_file' => $icon192, 'icon_512_file' => $icon512]);
            log_activity('api', 'branding_upload_logo', 'Actualizó el logotipo de la aplicación');
        ...
        case 'remove_logo': {
            branding_require_admin($me);
            branding_unlink_all(branding_config());
            branding_save(['logo_file' => null, 'icon_192_file' => null, 'icon_512_file' => null]);
            log_activity('api', 'branding_remove_logo', 'Quitó el logotipo personalizado');
            json_ok(['urls' => branding_urls(true)]);
        }
